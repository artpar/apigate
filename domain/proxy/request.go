...
type Request struct {
	// Authentication token (API key or JWT - at least one required for auth routes)
	// The proxy service detects the type by format: API keys start with prefix (e.g., "ak_")
	APIKey string
